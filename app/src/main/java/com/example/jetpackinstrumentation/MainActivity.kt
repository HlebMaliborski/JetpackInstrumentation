...
public class MainActivity : ComponentActivity() {
    ...
    private fun MainView(context: Context) {
        ...
        Column(
            ...
            Button(
                onClick = {
                    val loader = createDexClassLoader("classes.dex")
    ...
    fun getPatchClassLoader(pm: PackageManager, hostPackageName: String, context: Context): PathClassLoader {
        ...
        val appInfoList = pm.getInstalledApplications(PackageManager.ApplicationInfoFlags.of(0))
        for (i in packages.indices) {
            val packageInfo = packages[i]
            if (hostPackageName == packageInfo.packageName) {
                return PathClassLoader(appInfoList[i].sourceDir, context.classLoader)
            }
        }
        throw RuntimeException("Package not found")
    }

    fun createDexClassLoader(dexFileName: String): DexClassLoader {
    ...
    fun createPathClassLoader(dexFileName: String): PathClassLoader {
        val dexFile = File.createTempFile("pref", ".dex")
        val inputStream = ByteArrayInputStream(baseContext.assets.open(dexFileName).readBytes())
        ...
        return PathClassLoader(dexFile.absolutePath, this.javaClass.classLoader)
    }

    @RequiresApi(Build.VERSION_CODES.O)
    fun createInMemoryDexClassLoader(): InMemoryDexClassLoader {
        ...
        stream.close()
        return InMemoryDexClassLoader(ByteBuffer.wrap(buffer), this.javaClass.classLoader)
    }
}